...
func StartHandler(rid string) {
	...
	case WaterBearBiased:
		...
		if config.ThresholdMode() == int(hacss.LowThreshold) {
			log.Println("start LowThreshold")
		} else if config.ThresholdMode() == int(hacss.HighThreshold) {
			log.Println("start HighThreshold")
		}
	default:
